...
export class CommentCache extends BaseCache{
  ...
  public async getCommentsNameFromCache(postId:string):Promise<ICommentNameList[]>
  {
    try {
      ...
      const commmentsCount: number = await this.client.LLEN(`comments:${postId}`);
      const commments:string[] =await this.client.LRANGE(`comments:${postId}`,0,-1);

      const list:string[]=[];
      for(const item of commments){
        const comment : ICommentDocument = Helpers.parseJson(item) as ICommentDocument;
        list.push(comment.username);
      }
      const response:ICommentNameList  ={count:commmentsCount,
         names:list
      };
       return [response];
    } catch (error) {
