...
export class Get {
  ...
  public async reactionsByUsername(req: Request, res: Response): Promise<void> {
    ...
    res
      ...
      .json({ message: 'All reaction by username', reactions});
  }
  public async singleReactionByUsername(req: Request, res: Response): Promise<void> {
    ...
    res
      .status(HTTP_STATUS.OK)
      .json({ message: 'Single post reaction by username', reactions: reactions.length ? reactions[0] : {}, count: reactions.length ? reactions[1] : 0 });
  }
}
